...
void RK4::integrate() {
	...
	for (int i = 0; i < steps - 1; i++) {
		...
		rk_slopes[0].vel = - this->pos_arr[i] / (std::pow(this->pos_arr[i].len(), 3));
		...
		rk_slopes[1].vel = - pos_temp / (std::pow(pos_temp.len(), 3));
